...
### 4) 사용
- 사용자: 사이트 의견함에서 문의 작성
- 관리자: `https://내주소/admin` 접속 → 비밀번호 입력 → **방문 기록 / 의견함** 확인

방문 기록은 이름·IP를 남기지 않고, 익명 ID + 시각만 저장합니다. 일반 방문자는 볼 수 없고 `/admin`만 볼 수 있습니다.

> 이미 feedback 테이블만 만든 경우: SQL Editor에서 `visits` 테이블 생성 부분만 다시 실행하세요.
